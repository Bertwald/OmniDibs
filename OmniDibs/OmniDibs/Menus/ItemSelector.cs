...
namespace OmniDibs.Menus {
    internal class ItemSelector<T> where T : class {
        internal static T? SelectDatabaseItemFromMenu() {
            var items = DatabaseFacade.GetImmutableListOf<T>();
        ...
        internal static T? SelectItemFromList(List<T> items, string msg = "") {
            if (items == null || !items.Any()) {
                return null;
            }
            IndexMenu indexMenu = new( msg == "" ? typeof(T).Name: msg,
                                       items.Select(x => x.ToString()).ToList()!,
                                       0);
            int index = indexMenu.RunMenu();
            return items[index];
